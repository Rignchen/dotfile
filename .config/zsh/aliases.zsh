...
alias "pg_dump=docker run --name some-postgres --volume /var/lib/postgresql/data:/var/lib/postgresql/data -e POSTGRES_HOST_AUTH_METHOD=trust -d postgres > /dev/null && sleep 5 && docker exec -it some-postgres pg_dump -U postgres; docker rm -f some-postgres"
